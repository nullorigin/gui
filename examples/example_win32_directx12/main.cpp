...
int main(int, char **) {
  ...
  Win32_Init(hwnd);
  DX12_Init(g_pd3dDevice, NUM_FRAMES_IN_FLIGHT, DXGI_FORMAT_R8G8B8A8_UNORM,
            g_pd3dSrvDescHeap,
  ...
  while (!done) {
    ...
    DX12_RenderDrawData(Gui::GetDrawData(), g_pd3dCommandList);
    ...
    if (io.ConfigFlags & ConfigFlags_ViewportsEnable) {
      ...
      Gui::RenderPlatformWindowsDefault(nullptr, (void *)g_pd3dCommandList);
    }
