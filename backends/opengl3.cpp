...
//  PlatformIO interface. 2023-11-08: OpenGL: Update GL3W based
//  opengl3_loader.hpp to load "libGL.so" instead of "libGL.so.1",
...
//  C++11. 2022-09-27: OpenGL: Added ability to '#define OPENGL_DEBUG'.
...
//  #4832, #5127). 2022-05-13: OpenGL: Fixed state corruption on OpenGL ES 2.0
//  due to not preserving GL_ELEMENT_ARRAY_BUFFER_BINDING and vertex attribute
...
//  (opengl3_loader.hpp), removing requirement and support for
...
//  calling OpenGL3_CreateFontsTexture() before the first NewFrame()
...
//  2019-03-15: OpenGL: Added a GL call + comments in OpenGL3_Init()
...
//  opengl3.cpp/.h away from the old combined GLFW/SDL+OpenGL3
...
//  Added const char* glsl_version parameter to OpenGL3_Init() so user
...
//  io.RenderDrawListsFn callback and exposed SdlGL3_RenderDrawData()
...
#endif
#elif !defined(OPENGL_LOADER_CUSTOM)
...
// feature for this backend (opengl3.cpp):
// - You may need to regenerate opengl3_loader.hpp to add new symbols.
...
// extension
#ifndef OPENGL_ES2
#define OPENGL_USE_VERTEX_ARRAY
...
#define HAS_POLYGON_MODE
#endif

// Desktop GL 3.2+ has glDrawElementsBaseVertex() which GL ES and WebGL don't
// have.
#if !defined(OPENGL_ES2) && !defined(OPENGL_ES3) && defined(GL_VERSION_3_2)
#define OPENGL_MAY_HAVE_VTX_OFFSET
#endif

// Desktop GL 3.3+ and GL ES 3.0+ have glBindSampler()
#if !defined(OPENGL_ES2) && (defined(OPENGL_ES3) || defined(GL_VERSION_3_3))
#define OPENGL_MAY_HAVE_BIND_SAMPLER
#endif

// Desktop GL 3.1+ has GL_PRIMITIVE_RESTART state
#if !defined(OPENGL_ES2) && !defined(OPENGL_ES3) && defined(GL_VERSION_3_1)
#define OPENGL_MAY_HAVE_PRIMITIVE_RESTART
#endif

// Desktop GL use extension detection
#if !defined(OPENGL_ES2) && !defined(OPENGL_ES3)
#define OPENGL_MAY_HAVE_EXTENSIONS
#endif
...
struct OpenGL3_Data {
  ...
  OpenGL3_Data() { memset((void *)this, 0, sizeof(*this)); }
};
...
static OpenGL3_Data *OpenGL3_GetBackendData() {
  return Gui::GetCurrentContext()
             ? (OpenGL3_Data *)Gui::GetIO().BackendRendererUserData
             : nullptr;
}
...
#ifndef OPENGL_USE_VERTEX_ARRAY
struct OpenGL3_VtxAttribState {
...
bool OpenGL3_Init(const char *glsl_version) {
  ...
  OpenGL3_Data *bd = NEW(OpenGL3_Data)();
...
#ifdef OPENGL_MAY_HAVE_VTX_OFFSET
  if (bd->GlVersion >= 320)
    io.BackendFlags |=
...
#ifdef OPENGL_MAY_HAVE_EXTENSIONS
...
static void OpenGL3_SetupRenderState(DrawData *draw_data, int fb_width,
                                     int fb_height,
...
#ifdef OPENGL_MAY_HAVE_PRIMITIVE_RESTART
  if (bd->GlVersion >= 310)
    glDisable(GL_PRIMITIVE_RESTART);
#endif
#ifdef HAS_POLYGON_MODE
...
#ifdef OPENGL_MAY_HAVE_BIND_SAMPLER
...
void OpenGL3_RenderDrawData(DrawData *draw_data) {
  ...
  OpenGL3_VtxAttribState last_vtx_attrib_state_pos;
  last_vtx_attrib_state_pos.GetState(bd->AttribLocationVtxPos);
  OpenGL3_VtxAttribState last_vtx_attrib_state_uv;
  last_vtx_attrib_state_uv.GetState(bd->AttribLocationVtxUV);
  OpenGL3_VtxAttribState last_vtx_attrib_state_color;
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (pcmd->UserCallback == DrawCallback_ResetRenderState)
          OpenGL3_SetupRenderState(draw_data, fb_width, fb_height,
                                   vertex_array_object);
        else
...
static bool CheckShader(GLuint handle, const char *desc) {
  ...
  if ((GLboolean)status == GL_FALSE)
    fprintf(stderr,
            "ERROR: OpenGL3_CreateDeviceObjects: failed to compile "
            "%s! With GLSL: %s\n",
...
static bool CheckProgram(GLuint handle, const char *desc) {
  ...
  if ((GLboolean)status == GL_FALSE)
    fprintf(stderr,
            "ERROR: OpenGL3_CreateDeviceObjects: failed to link %s! "
            "With GLSL %s\n",
            desc, bd->GlslVersionString);
  if (log_length > 1) {
    Vector<char> buf;
...
static void OpenGL3_RenderWindow(Viewport *viewport, void *) {
