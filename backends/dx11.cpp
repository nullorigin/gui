...
//  access changed to bd->XXXX). 2021-05-19: DirectX11: Replaced direct access
...
//  dx11.cpp/.h away from the old combined DX11+Win32 example.
...
//  DX11_RenderDrawData() in the .h file so you can call it yourself.
//  2018-02-06: Misc: Removed call to Gui::Shutdown() which is not available
...
static DX11_Data *DX11_GetBackendData() {
  return Gui::GetCurrentContext()
             ? (DX11_Data *)Gui::GetIO().BackendRendererUserData
             : nullptr;
}

// Forward Declarations
static void DX11_InitPlatformInterface();
static void DX11_ShutdownPlatformInterface();

// Functions
static void DX11_SetupRenderState(DrawData *draw_data,
...
// Render function
void DX11_RenderDrawData(DrawData *draw_data) {
  ...
  if (ctx->Map(bd->pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource) != S_OK)
    return;
  DrawVert *vtx_dst = (DrawVert *)vtx_resource.pData;
  DrawIdx *idx_dst = (DrawIdx *)idx_resource.pData;
  ...
  // Create the vertex shader
  {
    ...
    D3D11_INPUT_ELEMENT_DESC local_layout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)offsetof(DrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
         (UINT)offsetof(DrawVert, uv), D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
         (UINT)offsetof(DrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0},
    };
...
void DX11_Shutdown() {
  ...
  DX11_ShutdownPlatformInterface();
  DX11_InvalidateDeviceObjects();
  if (bd->pFactory) {
    bd->pFactory->Release();
  }
...
static void DX11_CreateWindow(Viewport *viewport) {
  DX11_Data *bd = DX11_GetBackendData();
  DX11_ViewportData *vd = NEW(DX11_ViewportData)();
...
static void DX11_DestroyWindow(Viewport *viewport) {
  ...
  if (DX11_ViewportData *vd = (DX11_ViewportData *)viewport->RendererUserData) {
    if (vd->SwapChain)
...
static void DX11_SetWindowSize(Viewport *viewport, Vec2 size) {
  DX11_Data *bd = DX11_GetBackendData();
  DX11_ViewportData *vd = (DX11_ViewportData *)viewport->RendererUserData;
  if (vd->RTView) {
    vd->RTView->Release();
    vd->RTView = nullptr;
  }
  if (vd->SwapChain) {
    ...
    if (pBackBuffer == nullptr) {
      fprintf(stderr, "DX11_SetWindowSize() failed creating buffers.\n");
      return;
    }
    bd->pd3dDevice->CreateRenderTargetView(pBackBuffer, nullptr, &vd->RTView);
    pBackBuffer->Release();
...
static void DX11_RenderWindow(Viewport *viewport, void *) {
  DX11_Data *bd = DX11_GetBackendData();
  DX11_ViewportData *vd = (DX11_ViewportData *)viewport->RendererUserData;
  ...
  if (!(viewport->Flags & ViewportFlags_NoRendererClear))
    bd->pd3dDeviceContext->ClearRenderTargetView(vd->RTView,
                                                 (float *)&clear_color);
  DX11_RenderDrawData(viewport->DrawData);
}

static void DX11_SwapBuffers(Viewport *viewport, void *) {
  DX11_ViewportData *vd = (DX11_ViewportData *)viewport->RendererUserData;
...
static void DX11_InitPlatformInterface() {
  ...
  platform_io.Renderer_CreateWindow = DX11_CreateWindow;
  platform_io.Renderer_DestroyWindow = DX11_DestroyWindow;
  platform_io.Renderer_SetWindowSize = DX11_SetWindowSize;
  platform_io.Renderer_RenderWindow = DX11_RenderWindow;
  platform_io.Renderer_SwapBuffers = DX11_SwapBuffers;
}

static void DX11_ShutdownPlatformInterface() { Gui::DestroyPlatformWindows(); }
