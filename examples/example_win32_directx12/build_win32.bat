...
@REM Important: to build on 32-bit systems, the DX12 backends needs '#define TextureID U64', so we pass it here.
@set OUT_DIR=Debug
...
mkdir Debug
cl /nologo /Zi /MD /utf-8 %INCLUDES% /D TextureID=U64 /D UNICODE /D _UNICODE %SOURCES% /Fe%OUT_DIR%/%OUT_EXE%.exe /Fo%OUT_DIR%/ /link %LIBS%
