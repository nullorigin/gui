...
static uSynergyBool sSendReply(uSynergyContext *context) {
  ...
  // Send reply
  ret =
      context->m_sendFunc(context->m_cookie, context->m_replyBuffer, reply_len);
...
static void sSendJoystickCallback(uSynergyContext *context, uint8_t joyNum) {
  ...
  context->m_joystickCallback(context->m_cookie, joyNum,
                              context->m_joystickButtons[joyNum], sticks[0],
  ...
  if (memcmp(message + 4, "Synergy", 7) == 0) {
    ...
    for (; num_formats; num_formats--) {
      ...
      if (context->m_clipboardCallback)
        context->m_clipboardCallback(context->m_cookie, format, parse_msg,
                                     size);
...
static void sUpdateContext(uSynergyContext *context) {
  ...
  /*	If we didn't receive any data then we're probably still polling to get
     ...
     system with a Synergy thread that would hammer on polling, we let it rest
     for a bit if there's no data. */
  if (num_received == 0)
  ...
  /* Eat packets */
  for (;;) {
    /* Grab packet length and bail out if the packet goes beyond the end of the
     * buffer */
...
void uSynergySendClipboard(uSynergyContext *context, const char *text) {
  ...
  sAddUInt32(context,
             4 + 4 + 4 + text_length); /* Rest of message size: numFormats,
                                          format, length, data */
