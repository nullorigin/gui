...
#error ENABLE_FREETYPE_LUNASVG requires FreeType version >= 2.12
#endif
#endif
...
static void *FreeTypeDefaultAllocFunc(size_t size, void *user_data) {
  UNUSED(user_data);
  return ALLOC(size);
}
static void FreeTypeDefaultFreeFunc(void *ptr, void *user_data) {
  UNUSED(user_data);
  FREE(ptr);
}

// Current memory allocators
static void *(*GFreeTypeAllocFunc)(size_t size,
                                   void *user_data) = FreeTypeDefaultAllocFunc;
static void (*GFreeTypeFreeFunc)(void *ptr,
                                 void *user_data) = FreeTypeDefaultFreeFunc;
static void *GFreeTypeAllocatorUserData = nullptr;

// Lunasvg support
#ifdef ENABLE_FREETYPE_LUNASVG
static FT_Error LunasvgPortInit(FT_Pointer *state);
static void LunasvgPortFree(FT_Pointer *state);
static FT_Error LunasvgPortRender(FT_GlyphSlot slot, FT_Pointer *_state);
static FT_Error LunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache,
                                      FT_Pointer *_state);
#endif
...
struct FreeTypeFont {
  bool
  InitFont(FT_Library ft_library, const FontConfig &cfg,
           unsigned int
  ...
  FT_Face Face;
  unsigned int UserFlags; // = FontConfig::RasterizerFlags
  FT_Int32 LoadFlags;
...
bool FreeTypeFont::InitFont(FT_Library ft_library, const FontConfig &cfg,
  ...
  LoadFlags = 0;
  if ((UserFlags & FreeTypeBuilderFlags_Bitmap) == 0)
    LoadFlags |= FT_LOAD_NO_BITMAP;

  if (UserFlags & FreeTypeBuilderFlags_NoHinting)
    LoadFlags |= FT_LOAD_NO_HINTING;
  if (UserFlags & FreeTypeBuilderFlags_NoAutoHint)
    LoadFlags |= FT_LOAD_NO_AUTOHINT;
  if (UserFlags & FreeTypeBuilderFlags_ForceAutoHint)
    LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
  if (UserFlags & FreeTypeBuilderFlags_LightHinting)
    LoadFlags |= FT_LOAD_TARGET_LIGHT;
  else if (UserFlags & FreeTypeBuilderFlags_MonoHinting)
    LoadFlags |= FT_LOAD_TARGET_MONO;
  else
    LoadFlags |= FT_LOAD_TARGET_NORMAL;

  if (UserFlags & FreeTypeBuilderFlags_Monochrome)
    RenderMode = FT_RENDER_MODE_MONO;
  else
    RenderMode = FT_RENDER_MODE_NORMAL;

  if (UserFlags & FreeTypeBuilderFlags_LoadColor)
...
void FreeTypeFont::SetPixelHeight(int pixel_height) {
  ...
  // understand, currently Gui assumes that the 'pixel_height' is a maximum
  ...
  req.type = (UserFlags & FreeTypeBuilderFlags_Bitmap)
...
const FT_Glyph_Metrics *FreeTypeFont::LoadGlyph(uint32_t codepoint) {
...
#else
#if ((FREETYPE_MAJOR >= 2) && (FREETYPE_MINOR >= 12))
  ASSERT(slot->format != FT_GLYPH_FORMAT_SVG &&
         "The font contains SVG glyphs, you'll need to enable "
         "ENABLE_FREETYPE_LUNASVG in config.hpp and install required "
         "libraries in order to use this font");
#endif
  ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE ||
         slot->format == FT_GLYPH_FORMAT_BITMAP);
#endif // ENABLE_FREETYPE_LUNASVG
  ...
  if (UserFlags & FreeTypeBuilderFlags_Bold)
    FT_GlyphSlot_Embolden(slot);
  if (UserFlags & FreeTypeBuilderFlags_Oblique) {
    ...
    if (multiply_table == nullptr) {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
        for (uint32_t x = 0; x < w; x++)
          dst[x] = COL32(255, 255, 255, src[x]);
    } else {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
        for (uint32_t x = 0; x < w; x++)
          dst[x] = COL32(255, 255, 255, multiply_table[src[x]]);
    }
    break;
  }
                           ...
                           // each byte are ordered from MSB to LSB.
  {
    ...
    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) {
      ...
      for (uint32_t x = 0; x < w; x++, bits <<= 1) {
        if ((x & 7) == 0)
          bits = *bits_ptr++;
        dst[x] = COL32(255, 255, 255, (bits & 0x80) ? color1 : color0);
      }
    }
    break;
  }
...
#define DE_MULTIPLY(color, alpha)                                              \
  (U32)(255.0f * (float)color / (float)alpha + 0.5f)
    if (multiply_table == nullptr) {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
        for (uint32_t x = 0; x < w; x++) {
          uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4],
                  a = src[x * 4 + 3];
          dst[x] =
              COL32(DE_MULTIPLY(r, a), DE_MULTIPLY(g, a), DE_MULTIPLY(b, a), a);
        }
    } else {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) {
        for (uint32_t x = 0; x < w; x++) {
          uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4],
                  a = src[x * 4 + 3];
          dst[x] = COL32(multiply_table[DE_MULTIPLY(r, a)],
...
#ifndef DISABLE_RECT_PACK_IMPLEMENTATION
...
struct FontBuildSrcGlyphFT {
  GlyphInfo Info;
  uint32_t Codepoint;
  unsigned int
      *BitmapData; // Point within one of the dst_tmp_bitmap_buffers[] array

  FontBuildSrcGlyphFT() { memset((void *)this, 0, sizeof(*this)); }
};

struct FontBuildSrcDataFT {
  FreeTypeFont Font;
...
bool FontAtlasBuildWithFreeTypeEx(FT_Library ft_library, FontAtlas *atlas,
  ...
  Vector<FontBuildSrcDataFT> src_tmp_array;
  Vector<FontBuildDstDataFT> dst_tmp_array;
  ...
  for (int src_i = 0; src_i < atlas->ConfigData.Size; src_i++) {
    FontBuildSrcDataFT &src_tmp = src_tmp_array[src_i];
    ...
    src_load_color |=
        (cfg.FontBuilderFlags & FreeTypeBuilderFlags_LoadColor) != 0;
    FontBuildDstDataFT &dst_tmp = dst_tmp_array[src_tmp.DstIndex];
    src_tmp.SrcRanges =
  ...
  for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    ASSERT(sizeof(src_tmp.GlyphsSet.Storage.Data[0]) == sizeof(U32));
    const U32 *it_begin = src_tmp.GlyphsSet.Storage.begin();
    const U32 *it_end = src_tmp.GlyphsSet.Storage.end();
    for (const U32 *it = it_begin; it < it_end; it++)
      if (U32 entries_32 = *it)
        for (U32 bit_n = 0; bit_n < 32; bit_n++)
          if (entries_32 & ((U32)1 << bit_n)) {
            FontBuildSrcGlyphFT src_glyph;
            src_glyph.Codepoint = (Wchar)(((it - it_begin) << 5) + bit_n);
  ...
  Vector<unsigned char *> buf_bitmap_buffers;
  buf_bitmap_buffers.push_back(
      (unsigned char *)ALLOC(BITMAP_BUFFERS_CHUNK_SIZE));
  ...
  for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++) {
      FontBuildSrcGlyphFT &src_glyph = src_tmp.GlyphsList[glyph_i];
      ...
      ASSERT(buf_bitmap_current_used_bytes + bitmap_size_in_bytes <=
  ...
  stbrp_init_target(&pack_context, atlas->TexWidth - atlas->TexGlyphPadding,
                    ...
                    pack_nodes.Size);
  FontAtlasBuildPackCustomRects(atlas, &pack_context);
  ...
  if (src_load_color) {
    ...
    atlas->TexPixelsRGBA32 = (unsigned int *)ALLOC(tex_size);
    memset(atlas->TexPixelsRGBA32, 0, tex_size);
  } else {
    size_t tex_size = (size_t)atlas->TexWidth * atlas->TexHeight * 1;
    atlas->TexPixelsAlpha8 = (unsigned char *)ALLOC(tex_size);
  ...
  for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
      ...
      dst_font->AddGlyph(&cfg, (Wchar)src_glyph.Codepoint, x0, y0, x1, y1, u0,
                         v0, u1, v1,
                         info.AdvanceX * src_tmp.Font.InvRasterizationDensity);

      FontGlyph *dst_glyph = &dst_font->Glyphs.back();
      ...
      if (atlas->TexPixelsAlpha8 != nullptr) {
        ...
        for (int y = 0; y < info.Height;
          ...
          for (int x = 0; x < info.Width; x++)
            blit_dst[x] =
                (unsigned char)((blit_src[x] >> COL32_A_SHIFT) & 0xFF);
      } else {
...
static void *FreeType_Alloc(FT_Memory /*memory*/, long size) {
  return GFreeTypeAllocFunc((size_t)size, GFreeTypeAllocatorUserData);
}

static void FreeType_Free(FT_Memory /*memory*/, void *block) {
  GFreeTypeFreeFunc(block, GFreeTypeAllocatorUserData);
}
...
static void *FreeType_Realloc(FT_Memory /*memory*/, long cur_size,
  ...
  if (block == nullptr)
    return GFreeTypeAllocFunc((size_t)new_size, GFreeTypeAllocatorUserData);
  ...
  if (new_size > cur_size) {
    void *new_block =
        GFreeTypeAllocFunc((size_t)new_size, GFreeTypeAllocatorUserData);
...
static bool FontAtlasBuildWithFreeType(FontAtlas *atlas) {
  ...
  SVG_RendererHooks hooks = {LunasvgPortInit, LunasvgPortFree,
                             LunasvgPortRender, LunasvgPortPresetSlot};
  FT_Property_Set(ft_library, "ot-svg", "svg-hooks", &hooks);
#endif // ENABLE_FREETYPE_LUNASVG

  bool ret =
      FontAtlasBuildWithFreeTypeEx(ft_library, atlas, atlas->FontBuilderFlags);
  FT_Done_Library(ft_library);

  return ret;
}

const FontBuilderIO *FreeType::GetBuilderForFreeType() {
  static FontBuilderIO io;
  io.FontBuilder_Build = FontAtlasBuildWithFreeType;
  return &io;
}

void FreeType::SetAllocatorFunctions(
    void *(*alloc_func)(size_t sz, void *user_data),
    void (*free_func)(void *ptr, void *user_data), void *user_data) {
  GFreeTypeAllocFunc = alloc_func;
  GFreeTypeFreeFunc = free_func;
  GFreeTypeAllocatorUserData = user_data;
}
...
static FT_Error LunasvgPortRender(FT_GlyphSlot slot, FT_Pointer *_state) {
  ...
  // LunasvgPortPresetSlot(), the renderer hook still get called, so just
  // returns the error.
...
static FT_Error LunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache,
  ...
  // right before LunasvgPortRender(). If it's the latter, don't do
  ...
  slot->bitmap.rows = (unsigned int)(Ceil((float)box.h));
  slot->bitmap.width = (unsigned int)(Ceil((float)box.w));
  ...
  slot->metrics.width =
      FT_Pos(ROUND(metrics_width *
                   64.0)); // Using ROUND() assume width and height are positive
...
#endif // #ifdef ENABLE_FREETYPE_LUNASVG
